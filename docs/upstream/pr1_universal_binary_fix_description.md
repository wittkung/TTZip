...
- **Slice-Aware Dispatch**: On Darwin, each slice compiled during the build phase dynamically enables its native vector pipeline (`tbl.16b` / `vqtbl1q_u8` on ARM64, `_mm_shuffle_epi8` on x86_64).
