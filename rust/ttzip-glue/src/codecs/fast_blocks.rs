...
// MARK: - Snappy C-Bindings & Safe Interface
...
/// Computes maximum compressed buffer length for Snappy input size.
#[inline]
pub fn snappy_max_compressed_length(src_size: usize) -> usize {
    unsafe { sys_snappy::snappy_max_compressed_length(src_size) }
}

/// Parses uncompressed length from a Snappy varint header.
pub fn snappy_uncompressed_length(src: &[u8]) -> Result<usize, TTZipStatus> {
    ...
    let res = unsafe {
        sys_snappy::snappy_uncompressed_length(
            src.as_ptr() as *const libc::c_char,
            src.len(),
            &mut out_len,
        )
...
pub fn snappy_validate(src: &[u8]) -> bool {
    ...
    let res = unsafe {
        sys_snappy::snappy_validate_compressed_buffer(src.as_ptr() as *const libc::c_char, src.len())
    };
    res == SnappyStatus::Ok
}

/// Compresses memory block with Google Snappy.
pub fn snappy_compress(src: &[u8], dst: &mut [u8]) -> Result<usize, TTZipStatus> {
    if src.is_empty() {
        return Ok(0);
    }
    let mut out_len: libc::size_t = dst.len();
    let res = unsafe {
        sys_snappy::snappy_compress(
            src.as_ptr() as *const libc::c_char,
            src.len(),
            dst.as_mut_ptr() as *mut libc::c_char,
            &mut out_len,
    ...
    match res {
        SnappyStatus::Ok => Ok(out_len),
        SnappyStatus::BufferTooSmall => Err(TTZipStatus::ErrCompressionFailed),
        SnappyStatus::InvalidInput => Err(TTZipStatus::ErrInvalidParam),
    }
}

/// Decompresses memory block with Google Snappy.
pub fn snappy_decompress(src: &[u8], dst: &mut [u8]) -> Result<usize, TTZipStatus> {
    ...
    match res {
        SnappyStatus::Ok => Ok(out_len),
        SnappyStatus::BufferTooSmall => Err(TTZipStatus::ErrExtractionFailed),
        SnappyStatus::InvalidInput => Err(TTZipStatus::ErrCorruptHeader),
    }
}
